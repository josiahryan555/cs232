...
#            self.set_mmu_registers(cpu)     
    ...
    def _assign_proc_to_cpu(self, cpu):
        new_proc = self._ready_q.pop(0)
        if self._debug:
            print("Switching procs to {}".format(new_proc.get_name()))
        self._current_proc[cpu.get_num()] = new_proc
        cpu.set_registers(new_proc.get_registers())
        new_proc.set_state(PCB.RUNNING) 
#        self.set_mmu_registers(cpu)     
    ...
    #set_mmu_registers() sets the mmu relocation and limit registers to values in the os's currently running pcb
    def set_mmu_registers(self, cpu):
        reloc = self._current_proc[cpu.get_num()].get_low_mem()
        limit = self._current_proc[cpu.get_num()].get_high_mem() - reloc
        if reloc >= 0 and limit >= 0:
            cpu._mmu.set_reloc_register(reloc)  
            cpu._mmu.set_limit_register(limit)  
        else:
            print("reloc value of " + reloc + " or limit value of " + limit + " is too low.  Both must be >= 0")
