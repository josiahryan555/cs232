...
class CPU:
    ...
    def set_registers(self, registers):
        ...
        self._os.set_mmu_registers( self )
        ...
        while True:
            ...
            if self._debug:
                ...
                print("CPU {}: executing code at [{}]: {}".format(self._num, self._registers['pc'],
                                                          self._mmu.get_memory(self._registers['pc'])))

            # Execute the next instruction.
            self.parse_instruction(self._mmu.get_memory(self._registers['pc']))
    ...
    def parse_instruction(self, instr):
        ...
        if isinstance(instr, int):
            print(self._mmu.get_reloc_register())
            print(self._mmu.get_limit_register())
    ...
    def _get_value_at(self, addr):
        ...
        or hex.'''
        addr = eval(addr[1:])
        return self._mmu.get_memory(addr)
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.set_memory(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.set_memory(eval(dst), srcval)
    ...
    def handle_sub(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.get_memory(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.get_memory(eval(dst), srcval)
