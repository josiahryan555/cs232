...
class Monitor:
    ...
    def _load_program(self, startaddr, tapename, procname=None):
        ...
        try:
            with open(tapename, "r") as f:
                ...
                pcb.set_low_mem(addr)  # sets the pcb's low mem address
    ...
    def _handle_data_label(self, addr, line, pcb):
        ...
        """
        if len(line.split()) != 2:
            raise ValueError("Illegal format: __data: must be followed by entrypoint address.")
        high_mem_addr = addr + int(line.split()[1])  # addr (base addr) + line (size of file) = high mem location
        pcb.set_high_mem(high_mem_addr)
        if self._debug:
            print("__data found at physical location", addr, "and goes up to", high_mem_addr)
