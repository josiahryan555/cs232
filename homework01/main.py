...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: run a batch of programs who's starting location are in an array at location <addr>")
                ...
                elif instr.upper().startswith('R '): #runs batch programming
                    self._set_batch_program_addr(arg1) #sets the batch address to the given address, and runs the programs in the batch
    ...
    #description: sets the address of the batch programming array, then executes those programs
    #parameters: address - the address of the array of programs to be batched
    def _set_batch_program_addr(self, addr):
        # creates a new thread, passing in memory address stored at addr, the os, and the
        # starting address
        self._cpu = CPU(self._ram, calos.CalOS(), self._ram.__getitem__(addr), self._debug) 
        self._cpu.set_batch_program_addr(addr)  #sets address of the array of programs
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
