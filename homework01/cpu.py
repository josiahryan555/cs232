...
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, num=0):
        ...
        self._os = os
        self._debug = debug
        self._batch = False  #sets batching to False as a default
        self._batch_program_addr = 1000 #declares the batch program address (1000 is default)
    ...
    #description: sets the batch program address, and switches to batch mode
    #parameters: address of array of program starts
    def set_batch_program_addr(self, addr):
        self._batch = True
        self._batch_program_addr = addr
    ...
    def run(self):
        ...
        if (self._batch == False):  #if batching mode is off, it runs normally (runs the one program and stops)
            self._run_program()
        else:  # batching mode is on: will run each program in order according to the array based in
            address = True #address is valid
            while (address == True): # loops while the address is valid
                self._run_program() #run that program
                if self._ram.__getitem__(self._batch_program_addr+1): #if there is another address in the batch array
                    self._batch_program_addr += 1 #moves to next address in batching array
                    self.set_pc(self._ram.__getitem__(self._batch_program_addr)) # set pc to new program
                else:  #if there are no programs in the bash
                    address = False #stops loop 
...
#ram = RAM()
#cpu = CPU(ram, calos.CalOS(), 0, False)
#CPU.set_batch_program_addr(CPU, 1000)