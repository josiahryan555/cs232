...
public class Client {
  ...
  public static void main(String [] args) {
     Client me = new Client(args);
     me.run();
  }

  /* constructor
   * @param: args, an array of Strings
   * Precondition: args.length == 1 or 2 &&
   *                args[0] is the name of an input file
   *                 whose first line is N, an integer
   *                 followed by a blank line
   *                 followed by N integers, one per line.
   *                 if args[1], args[1] is an int number of
   *                 threads that should execute the code.
  ...
  // given the command line arguments, returns the number of threads to run the program with
  // Paramaters: command line args[]
  // Returns   : number of threads to execute the program with
